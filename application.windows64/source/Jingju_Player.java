...
public class Jingju_Player extends PApplet {
...
String itFontFile = "Roboto-Italic.ttf";
PFont zhFont;
PFont enFont;
PFont itFont;
...
public void setup() {
  ...
  zhFont = createFont(zhFontFile, lineSize);
  enFont = createFont(enFontFile, lineSize);
  itFont = createFont(itFontFile, lineSize);
  
  // Title and sections
  titleTable = loadTable(titleFile);
  title = titleTable.getString(0, 0);
  transTitle = new String[3];
  for (int i = 1; i < 4; i++) {
    transTitle[i-1] = titleTable.getString(0, i);
  }
  ...
  transLines = new String[linesTable.getRowCount()];
  transLineX = 0;
  for (int i = 0; i < linesTable.getRowCount(); i++) {
    lines[i] = linesTable.getString(i, 0);
    textFont(zhFont);
    textSize(lineSize);
    if (textWidth(lines[i]) > transLineX) {
      transLineX = textWidth(lines[i]);
    }
    linesStart[i] = PApplet.parseInt(linesTable.getFloat(i, 1) * 1000);
    linesEnd[i] = PApplet.parseInt(linesTable.getFloat(i, 2) * 1000);
    transLines[i] = linesTable.getString(i, 3);
  }
  transLineX += 5;
  ...
  for (int i = 0; i < banshiStart.length; i++) {
    ...
    banshiStart[i] = PApplet.parseInt(banshiTable.getFloat(i, 1) * 1000);
    banshiEnd[i] = PApplet.parseInt(banshiTable.getFloat(i, 2) * 1000);
  }
  ...
  for (int i = 0; i < tempoData.getRowCount(); i++) { 
    tempoTime[i+1] = PApplet.parseInt(tempoData.getFloat(i, 0)*1000);
    tempoValue[i+1] = tempoData.getFloat(i, 1);
    float b = tempoData.getFloat(i, 2); // converts the beat value into string
    if (b % 1 > 0) {
      beat[i+1] = PApplet.parseInt(str(b).charAt(str(b).length()-1))-48;
    } else {
      beat[i+1] = 1;
    }
  }
  ...
  tempoValue[tempoValue.length-1] = 0.0f;
  ...
  int maxLines = PApplet.parseInt((height-lyricsBoxH) / lineShift);
...
public void draw() {
  ...
  textSize(30);
  fill(0);
  text(title, tempoBoxX, 55);
  String aria = transTitle[0]+" ";
  String play = " "+transTitle[1];
  String character = transTitle[2];
  textFont(enFont);
  textSize(subTitleSize);
  text(aria, tempoBoxX+20, 90); 
  textFont(itFont);
  textSize(subTitleSize);
  text(play, tempoBoxX+20+textWidth(aria), 90);
  textFont(enFont);
  textSize(subTitleSize);
  text(character, tempoBoxX+20+textWidth(aria)+textWidth(play), 90);
  ...
  if (voicePlayer.isPlaying()) {
    ...
    strokeWeight(1);
    fill(0, 200, 0);
    rect(playX-butRad*0.4f, playY-butRad*0.4f, butRad*0.325f, butRad*0.8f);    
    rect(playX+butRad*0.075f, playY-butRad*0.4f, butRad*0.325f, butRad*0.8f);
  } else {
    stroke(0);
    strokeWeight(1);
    fill(0, 150, 0);
    triangle(playX-butRad*0.2f, playY-butRad*0.4f, playX-butRad*0.2f, playY+butRad*0.4f, playX+butRad*0.4f, playY);
  }
  // Stop button
  ...
  strokeWeight(1);
  fill(175, 0, 0);
  rect(stopX-butRad*0.325f, stopY-butRad*0.325f, butRad*0.65f, butRad*0.65f);
  ...
  stroke(0);
  int voiceVolumeFill = PApplet.parseInt(map(voiceVol, volMax, volMin, 255, 0));
  ...
  int accVolumeFill = PApplet.parseInt(map(accVol, volMax, volMin, 255, 0));
  fill(accVolumeFill);  
  ...
  text(tempoMark, tempoBoxX, height-tmY, textWidth(tm0)*1.1f, tms*1.2f);
  ...
  tlX = tempoBoxX+textWidth(tm0)*1.1f+5+tlr;
  tlY = height-tmY+(tms*0.6f);
  ...
  for (int i = 0; i < banshiStart.length; i++) {
    ...
    fill(0);
    String banshiLabel = banshi[i];
    if (textWidth(banshiLabel) < banshiHeaderW) {
      textFont(zhFont);
      textSize(banshiHeaderH*0.75f);
      text(banshiLabel, banshiHeaderX+banshiHeaderH*0.05f, height-tempoBoxX-tempoBoxH+banshiHeaderH*0.8f);
    } else {
      textFont(enFont);
      textSize(banshiHeaderH*0.75f);
      text("#"+str(i+1), banshiHeaderX+banshiHeaderH*0.05f, height-tempoBoxX-tempoBoxH+banshiHeaderH*0.8f);
    }
  }
  ...
  for (int i = skipTop; i < lines.length-skipBottom; i++) {
    ...
    textFont(enFont);
    text(transLines[i], lineX+lineSize*8+lineAdjust+transLineX, lyricsBoxY+lineShift*(i-skipTop+1));
  }
  ...
  if (linesSkip > 0) {
    ...
    if (tempoInstant > linesStart[linesStart.length-linesSkip-1]) {
      ...
      float tY1 = lyricsBoxY+skipBarH*0.2f; // triangle Y
      float tY2 = lyricsBoxY+skipBarH*0.8f; // triangle Y
      fill(120, 70, 15, 200);
      triangle(tX, tY1, tX-(skipBarH*0.8f)/2, tY2, tX+(skipBarH*0.8f)/2, tY2);
    } else {
      int skipBarY = height-tempoBoxX-tempoBoxH-tempLyrSp-skipBarH;
      fill(120, 70, 15, 50);
      rect(lyricsBoxX, skipBarY, lyricsBoxW, skipBarH);
      float tY1 = skipBarY+skipBarH*0.2f; // triangle Y
      float tY2 = skipBarY+skipBarH*0.8f; // triangle Y
      fill(120, 70, 15, 200);
      triangle(tX, tY2, tX-(skipBarH*0.8f)/2, tY1, tX+(skipBarH*0.8f)/2, tY1);
    }
  }
...
public void mouseClicked() {
  ...
  // Tempo box
      ...
      int targetPos = PApplet.parseInt(map(mouseX, tempoBoxX, width-tempoBoxX, 0, voicePlayer.length()));
...
public void mouseDragged() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--hide-stop", "Jingju_Player" };
    if (passedArgs != null) {
